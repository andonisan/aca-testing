...
#Secrets
$pat = "GitHub PAT token" #GitHub PAT token
$acrPassword = "The Admin Account `Password` on the ACR" #The Admin Account `Password` on the ACR

#Load user Secrets not in repository
. .\local.env.ps1
...
az containerapp create --resource-group "$acaResourceGroupName" `
    ...
    --scale-rule-metadata  "queueLength=1" "queueName=gh-runner-scaler" `
